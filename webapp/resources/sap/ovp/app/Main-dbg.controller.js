sap.ui.define(["sap/ui/core/mvc/Controller", "sap/m/library", "sap/ui/model/odata/ODataUtils", "sap/ovp/cards/generic/Component",
    ...
    function (Controller, SapMLibrary, ODataUtils, Component, NavigationHandler, MessageBox, CommonUtils, PersonalizationUtils,
              ...
              TemplateBaseExtension, ControllerExtension, OVPUtils) {
        ...
        return Controller.extend("sap.ovp.app.Main", {
                ...
                for (key in this.aOrderedCards) {
                    ...
                    if (oCardItemsBinding) {
                        ...
                        try {
                            ...
                            jQuery.sap.log("applying filter on a card which is not valid");
                        }
                ...
                // invocation
                        ...
                        for (var i = 0, j = 0; j < aVisibleCards.length; i++, j++) {
                            ...
                            if (oCard.settings.tabs) {
                                ...
                                } else if (this.aOrderedCards[i].selectedKey && oCard.settings.tabs.length >= this.aOrderedCards[i].selectedKey) {
            ...
            _createModelViewMap: function (oCard) {
                ...
                setOrderedCardsSelectedKey: function (sCardId, nSelectedKey) {
                    ...
                    for (var i = 0; i < aOrderedCards.length; i++) {
                        if (aOrderedCards[i] && aOrderedCards[i].id == sCardId) {
                            this.aOrderedCards[i].selectedKey = nSelectedKey;
                            break;
                        }
                    }
                },
                ...
                getOrderedCardsSelectedKey: function (sCardId) {
                    ...
                    for (var i = 0; i < aOrderedCards.length; i++) {
                        if (aOrderedCards[i] && aOrderedCards[i].id == sCardId) {
                            return this.aOrderedCards[i].selectedKey;
                        }
                    }
                },
            ...
            verifyGlobalFilterLoaded: function () {
                ...
                this.getView().byId("ovpCardPage").setVisible(false);
		...
		//triggering a async call so that card components should be created
            ...
            savePersonalization: function (oChange) {
                ...
                oChange.forEach(function (oOneChange) {
                    ...
                    aChanges.push({
                        selectorControl: this.getView().byId(sCardId),
                        changeSpecificData: oOneChange
                    });
                    aCardForDeletion.push(this.getView().byId(sCardId));
                    aChangeTypeForDeletion.push(oOneChange.changeType);
                }.bind(this));
                ControlPersonalizationAPI.resetChanges(aCardForDeletion,aChangeTypeForDeletion).finally(function(){
                    ControlPersonalizationAPI.addPersonalizationChanges({
                        controlChanges: aChanges
                    },true).then(function (aChanges) {
                        ControlPersonalizationAPI.saveChanges(aChanges, layout).then(function () {
                            ...
                            if (oCard) {
                                ...
                                if (oCard.settings.tabs) {
                                    ...
                                    if (nIndex && typeof nIndex == "number" && nIndex <= oCard.settings.tabs.length && nIndex !== nCurrentIndex) {
                                        iIndex = nIndex >= 1 ? nIndex - 1 : nIndex;
                                        this.setOrderedCardsSelectedKey(oCard.id, nIndex);
                                    } else if (this.aOrderedCards[j].selectedKey && oCard.settings.tabs.length >= this.aOrderedCards[j].selectedKey) {
                                        iIndex = this.aOrderedCards[j].selectedKey - 1;
                                    }
                                    this.initializeTabbedCard(
                                        oCard, iIndex);
                                }
            ...
            rerenderCards: function (cardArray) {
                var aDialogCards = cardArray;
                this.createOrDestroyCards.apply(this, [this.aOrderedCards, aDialogCards, false]);
                ...
                for (var i = 0; i < this.aOrderedCards.length; i++) {
                    var oCard = this.aOrderedCards[i];
            ...
            _onCardMenuButtonPress: function () {
                ...
                var bIsFiori3Enabled = this.getUIModel() && this.getUIModel().getProperty("/enableFiori3");

                var oCancelButton = new Button("manageCardsCancelBtn", {
                    text: OvpResources.getText("cancelBtn"),
                    type: bIsFiori3Enabled ? "Transparent" : "Default",
                    press: function () {
                        this.oDialog.close();
                    }.bind(this)
                });
                        ...
                        MessageBox.show(OvpResources.getText("reset_cards_confirmation_msg"), {
                            ...
                            onClose: function (oAction) {
                                if (oAction === MessageBox.Action.OK) {
                                    ...
                                    this.createOrDestroyCards.apply(this, [this.aOrderedCards, this.aManifestOrderedCards, (sLayoutName === "sap.ovp.ui.DashboardLayout") ? true : false]);
                ...
                this.oDialog = new Dialog({
                    ...
                    content: oCardsTable,
                    buttons: bIsFiori3Enabled ? [oSaveButton, oResetButton, oCancelButton] : [oSaveButton, oCancelButton, oResetButton],
                    afterClose: function () {
                        this.visibilityChanges = [];
			...
			this.getGlobalFilter().setFilterData({_CUSTOM : oCustomSelectionVariantData});
                }
                var oUiState = oGlobalFilter.getUiState({
                    allFilters: false
                });
