	...
	var AnnotationHelper = {
		...
		* @returns {String} Returns expression binding or boolean value based on vRawValue, viewLevel
		...
		getDeleteButtonVisibility: function (oRawValue, viewLevel, oDeleteHidden) {
			if (viewLevel > 1) {
				if (typeof oDeleteHidden === 'object' && oDeleteHidden.$Path) {
					return "{= !${" + oDeleteHidden.$Path + "} && (${ui>/editable} === 'Editable')}";
				}
			...
			} else if (typeof oDeleteHidden === 'object' && oDeleteHidden.$Path) {
				return "{= !${" + oDeleteHidden.$Path + "} && !(${ui>/editable} === 'Editable')}";
		...
		* @returns {String} Returns expression binding or boolean value based on vRawValue & oDraftNode
		...
		getEditAction: function (oEntitySet) {
			...
			var bStickySession = oAnnotations.hasOwnProperty('@com.sap.vocabularies.Common.v1.StickySessionSupported');
			if (bDraftRoot) {
				return sPath + '@com.sap.vocabularies.Common.v1.DraftRoot/EditAction';
			} else if (bStickySession) {
				return sPath + '@com.sap.vocabularies.Common.v1.StickySessionSupported/EditAction';
			}
		},
