		...
		var AnnotationHelper = {
			...
			createBindingForDraftAdminBlock: function (oMetaModel, sEntityType, sFormatter) {
				...
				return oMetaModel.requestObject(sPath).then(function (oDADEntityType) {
					var sBinding = "{parts: [{path: 'InProcessByUser'}, " +
			...
			checkForActions: function (aLineItems) {
				...
				for (var i = 0; i < aLineItems.length; i++) {
					...
					if ((oLineItem["$Type"] === "com.sap.vocabularies.UI.v1.DataFieldForAction" ||
						(oLineItem["$Type"] === "com.sap.vocabularies.UI.v1.DataFieldForIntentBasedNavigation" && (oLineItem.RequiresContext && oLineItem.RequiresContext.Bool === "true"))) &&
				...
				for (i in aRestrictedProperties) {
					...
					if (oNavigationProperty.NavigationProperty.$NavigationPropertyPath === sCurrentCollectionName && oNavigationProperty.InsertRestrictions) {
						if (oNavigationProperty.InsertRestrictions.Insertable && oNavigationProperty.InsertRestrictions.Insertable.$Path) {
							return '{= %{' + oNavigationProperty.InsertRestrictions.Insertable.$Path + '} ? ${ui>/editable} === \'Editable\'  : false }';
						}
			...
			**/
			isNavigationPropertyDeletable: function (sCurrentCollectionName, aRestrictedProperties, bDeletable) {
				...
				for (i in aRestrictedProperties) {
					oNavigationProperty = aRestrictedProperties[i];
					if (oNavigationProperty.NavigationProperty.$NavigationPropertyPath === sCurrentCollectionName && oNavigationProperty.DeleteRestrictions) {
