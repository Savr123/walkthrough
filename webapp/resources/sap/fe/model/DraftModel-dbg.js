...
sap.ui.define([
	...
	"sap/base/Log"
], function (JSONModel, ODataListBinding, Context, Filter, ManagedObject, ChangeReason, ResourceModel, testableHelper, ODataContextBinding, Log) {
	"use strict";
	...
	function mergeEditStateFilterToFilter(sEditState, sFilter) {
		var sFilterValue = getFilterForEditState(sEditState, ""), aResult, sEditStateFilter;
		if (sFilter) {
	...
	function _upgrade(oModel) {
		...
		function upgradeContext(oContext) {
			var bFoundDraftEntitySet = false, sPath = oContext.getPath();
			...
			if (getData(oModel, "bUpgraded") && sPath) {
				aDraftEntitySets.forEach(function (entitySet) {
					...
					var isDraftEntitySetPath = !bFoundDraftEntitySet && sPath.substring(sPath.indexOf('/') + 1, sPath.indexOf('(')) === entitySet["@sapui.name"];
