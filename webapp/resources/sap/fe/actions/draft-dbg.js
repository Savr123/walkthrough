...
sap.ui.define(["sap/m/Dialog", 'sap/m/Button', 'sap/m/Text', 'sap/m/MessageBox', 'sap/fe/actions/messageHandling'],
	function(Dialog, Button, Text, MessageBox, messageHandling) {
		'use strict';
		...
		function createDraftFromActiveDocument(oContext, mParameters) {
			var	mParam = mParameters || {},
				localI18nRef =  sap.ui.getCore().getLibraryResourceBundle("sap.fe"),
			...
			return Promise.resolve(mParam.fnBeforeCreateDraftFromActiveDocument ? mParam.fnBeforeCreateDraftFromActiveDocument(oContext, bRunPreserveChangesFlow) : true).then(function (bExecute) {
				...
				return oContext.executeDraftEditAction(bRunPreserveChangesFlow).catch(function(oResponse) {
					...
					if (bRunPreserveChangesFlow && oResponse.status === 409) {
						return Promise.resolve(mParam.fnWhenDecisionToOverwriteDocumentIsRequired ? mParam.fnWhenDecisionToOverwriteDocumentIsRequired() : true).then(overwriteOnDemand);
					} else {
