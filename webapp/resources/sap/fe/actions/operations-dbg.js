...
sap.ui.define([
	...
	function (MessageBox, Dialog, JSONModel, XMLTemplateProcessor, XMLPreprocessor, Fragment, messageHandling) {
		'use strict';
		...
		function showActionParameterDialog(sActionLabel, aActionParameters, oActionContext, oParentControl, mParameters) {
			var sPath = '/' + oActionContext.getPath().split('/')[1],
			...
			function setActionParameterDefaultValue() {
				...
				if (aContexts.length) {
					return new Promise(function(resolve, reject) {
						...
						// additional side effect to request additional property paths such as text assoications of the actual target properties
						if (mParameters.additionalSideEffect) {
							aContexts[0].requestSideEffects(mParameters.additionalSideEffect).then(
								function () {
									// success
								},
								function () {
		...
		/**
			 ...
			 * @function
			 * @name sap.fe.actions.operations.setUserDefaults
			 * @memberof sap.fe.actions.operations
				...
				return oCrossAppNav.getStartupAppState(oAppComponent).then(function(oStartupAppState) {
					...
					aActionParameters.map(function(oParameter) {
						if (oStartupParameters[oParameter.$Name]) {
							...
							if (range && range.length === 1 && range[0].Sign === 'I' && range[0].Option === 'EQ') { // consider only single values
