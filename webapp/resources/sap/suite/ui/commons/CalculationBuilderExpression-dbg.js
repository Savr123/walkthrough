sap.ui.define([
	...
	CalculationBuilderExpression.prototype._setItems = function (aItems) {
		var aOldItems = this.getItems(),
			iOldLength = aOldItems.length;

		aItems = aItems || [];

		for (var i = 0; i < aItems.length && i < iOldLength; i++) {
			var oOldItem = aOldItems[i];
			oOldItem._reset();
			oOldItem.setProperty("key", this._getKeyFromCreatedItem(aItems[i]), true);
		}

		if (i < iOldLength) {
			for (; i < iOldLength; i++) {
				this.removeAggregation("items", aOldItems[i], true);
			}
		} else if (i < aItems.length) {
			for (; i < aItems.length; i++) {
				this.addAggregation("items", this._convertFromNewItem(aItems[i]), true);
			}
		}
	};
