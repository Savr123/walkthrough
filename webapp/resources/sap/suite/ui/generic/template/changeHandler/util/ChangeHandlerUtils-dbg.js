...
],function(AnnotationHelper, AnnotationHelperForDesignTime) {
	"use strict";
		...
		if (!ChangeHandlerUtils.fnIsSubsectionsPresent(aAnnotations[iTargetSectionIndex])) {
			...
			if (aFacet[0].RecordType === REFERENCE_FACET){
				...
				if (aAnnotations[iSourceSectionIndex].Facets) {
					...
					var oSourceCollectionFacet = ChangeHandlerUtils.createCollectionFacets([aAnnotations[iSourceSectionIndex]], aAnnotations[iSourceSectionIndex].Label.String);
				}
			...
			var oNewCollectionFacet = ChangeHandlerUtils.createCollectionFacets(oResultantCollectionFacet, aAnnotations[iTargetSectionIndex].Label.String);
