sap.ui.define([
...
], function(AnnotationChangeUtils, ChangeHandlerUtils, AnnotationHelper) {
	"use strict";
	...
	DesigntimeUtils.modifyDataPointForChart = function(sTarget, oEntityType, oNewMeasure, aCustomChanges) {
		...
		if (sAnnotationPath) {
			...
			if (oNewMeasure.Measure && oNewMeasure.Measure.PropertyPath && oNewMeasure.Measure.PropertyPath !== "") {
				...
				if (!oNewMeasure.DataPoint || jQuery.isEmptyObject(oNewMeasure.DataPoint)) {
					return;
				}
		...
		if (oOldDataPoint) {
			oNewDataPoint = jQuery.extend( true,{}, oOldDataPoint, oNewMeasure.DataPoint);
		} else {
			oNewDataPoint = jQuery.extend( true, DesigntimeUtils.createNewDataPointForChart(), oNewMeasure.DataPoint);
		}
	 ...
	 * @public
	 */
	DesigntimeUtils.createNewChart = function(sDataPointQualifier) {
