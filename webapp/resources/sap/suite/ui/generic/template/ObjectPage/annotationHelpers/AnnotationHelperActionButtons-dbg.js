...
sap.ui.define(["sap/base/Log", "sap/ui/model/odata/AnnotationHelper", "sap/suite/ui/generic/template/js/AnnotationHelper", "sap/suite/ui/generic/template/lib/testableHelper"], function(Log, AnnotationHelperModel, AnnotationHelper, testableHelper) {
	...
	function getCallAction(oInterface, oDataField){
		...
		var sLabel = AnnotationHelperModel.format(oInterface, oDataField.Label) || "";
		var sInvocationGrouping = (oDataField.InvocationGrouping && oDataField.InvocationGrouping.EnumMember) || "";
		// sAction and sInvocationGrouping are technical identifiers, so they should not contain any 'special' characters. However, sLabel is a string in natural language, which might contain the '. Therefore, this needs to be escaped by a preceeding \.
		return "._templateEventHandlers.onCallAction('" + sAction + "', '" + sLabel.replace("'", "\\'") + "', '" + sInvocationGrouping + "')";
	}
