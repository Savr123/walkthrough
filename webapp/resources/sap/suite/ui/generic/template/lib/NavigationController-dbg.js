...
sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		function fnGetRouterInput(oTreeNode, aKeys, mAppStates){
			...
			var bIsQuery = !jQuery.isEmptyObject(mAppStates);
		...
		function fnCreateLinkInfoForNode(oTreeNode, aAppStatePromises){
			...
			oRet.navigate = function(iDisplayMode){
				oTemplateContract.oBusyHelper.setBusy(oAppStatePromise.then(function(){
					...
					fnNavigateToIdentity(oNewIdentity);
				}));
			};
		...
		function getAppStatePromiseForUpNavigation(oTargetTreeNode, mAppStates){
			return (oTargetTreeNode.fCLLevel === 0 || oTargetTreeNode.fCLLevel === 3) ? getApplicableStateForComponentAddedPromise(oTargetTreeNode.componentId, null, mAppStates) : oTemplateContract.oFlexibleColumnLayoutHandler.getAppStatesPromiseForColumnClose(oTargetTreeNode, mAppStates);
		}
		...
		function fnNavigateAfterActivation(oActiveContext){
			...
			var oAppStatePromise = getAppStatePromiseForUpNavigation(oTargetIdentity.treeNode, oTargetIdentity.appStates);
		...
		function fnNavigateUpAfterDeletion(iTargetViewLevel){
			...
			// fill the app states
			var oNavigationPromise = getAppStatePromiseForUpNavigation(oTargetIdentity.treeNode, oTargetIdentity.appStates).then(function(){
