sap.ui.define(["sap/ui/base/Object",
	...
	"sap/ui/model/analytics/odata4analytics"
				...
				NavigationHandler, NavError, testableHelper, Log, odata4analytics) {
	"use strict";
		...
		function fnSetEnabledToolbarButtons (oSubControl) {
			...
			if (aToolbarControls && aToolbarControls.length > 0) {
				for (var i = 0; i < aToolbarControls.length; i++) {
					...
					if (oToolbarControl.getMetadata().getName() === "sap.m.Button") {
		...
		function fnHandleToolbarButtonsEnablement (oToolbarControl, oModel, aContexts, oControl) {
			...
			if (mCustomData && mCustomData.Type) {
				...
				if (mCustomData.originalButtonId) {
					...
					oToolbarControl = oView.byId(sLocalButtonId);
				} else {
				...
				if (oComponentUtils.isDraftEnabled()) {
					...
					oView.attachBrowserEvent(
							...
							function(oBrowserEvent) {
								...
								var isRowAction = oBrowserEvent.target.id.indexOf("rowAction") > -1 ? true : false;
								var isColumnListItem = oBrowserEvent.target.id.indexOf("ColumnListItem") > -1 ? true : false;
								// CTRL key is checked with the ENTER key as CTRL + ENTER is used as a shortcut for adding entries to a table
								if (oBrowserEvent.keyCode === 13 && oBrowserEvent.ctrlKey !== true && oView.getModel("ui").getProperty("/editable") && !isSearchField && !isTextArea && !isRowAction && !isColumnListItem) {
