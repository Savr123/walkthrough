	...
	function(BaseObject, Device, JSONModel, MessageToast, ActionSheet, Dialog, Popover, deletionHelper,
		...
		function getMethods(oTemplateContract) {
			...
			function createNonDraft(sEntitySet, vPredefinedValues) {
				return CRUDHelper.createNonDraft("/" + sEntitySet, oTemplateContract.oAppComponent.getModel(), setEditableNDC, fnRegisterNonDraftCreateContext.bind(null, sEntitySet), vPredefinedValues);
			}
			...
			*/
			...
			// returns a Promise that resolves to a function that performs the navigation which has to be done after cancelling a draft
