sap.ui.define(["sap/ui/base/Object"], function (BaseObject) {
		...
		function getMethods(oAppComponent) {
			...
			function registerContext(oContext, iViewLevel, sEntitySet) {
				...
				mPath2ContextData[sPath] = jQuery.extend(mPath2ContextData[sPath], {
				...
				oContextData.oEditingPromise = new Promise(function (fnResolve, fnReject) {
					...
					oEditingPromise.then(function (oEditInfo) {
						if (oEditInfo.draftAdministrativeData) {
							...
							// Extend the draft object with to active context which is already known in this case
							// When getDraftSiblingPromise method check for the existance of sActiveContextPath and in the
							// presence of the same return context back and avoids a sibling backend call
