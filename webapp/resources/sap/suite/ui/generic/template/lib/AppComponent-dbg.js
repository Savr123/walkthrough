...
sap.ui.define([
	...
	"sap/base/Log"
], function(UIComponent, NavContainer, FlexibleColumnLayout, Filter, FilterOperator, MessageScope, JSONModel, ResourceModel, ApplicationController,
	...
	CommonMethods, TemplateLibrary, Log) {
	"use strict";
	...
	function getMethods(oAppComponent, oAppId) {
		...
		function init(){
			...
			oMessageScopSupportedPromise.then(function(bMessageScopeSupported){
				if (bMessageScopeSupported){
					...
					oModel.enableCanonicalRequests(true);
		...
		function fnNormalizeTableSettings(oPage) {
			// 1. map boolean settings gridTable and treeTable to tableType
			oPage.component.settings = oPage.component.settings || {};
			oPage.component.settings.tableType = oPage.component.settings.tableType || (oPage.component.settings.gridTable ? "GridTable" : undefined);
			oPage.component.settings.tableType = oPage.component.settings.tableType || (oPage.component.settings.treeTable ? "TreeTable" : undefined);

			// 2. map flat settings to structured ones
			oPage.component.settings.tableSettings = oPage.component.settings.tableSettings || {};
			oPage.component.settings.tableSettings.type = oPage.component.settings.tableSettings.type || oPage.component.settings.tableType;
			oPage.component.settings.tableSettings.multiSelect = (oPage.component.settings.tableSettings.multiSelect === undefined ? oPage.component.settings.multiSelect : oPage.component.settings.tableSettings.multiSelect);

			// since we still use obsolete properties in the fragments, set also obsolete properties... (TODO: will be removed later)
			oPage.component.settings.tableType = oPage.component.settings.tableSettings.type;
			oPage.component.settings.multiSelect = oPage.component.settings.tableSettings.multiSelect;
		}
