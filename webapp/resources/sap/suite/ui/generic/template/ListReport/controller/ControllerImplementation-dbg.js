...
sap.ui.define([
	...
	"sap/base/util/ObjectPath"
		...
		WorklistHandler, ShareUtils, Log, ObjectPath) {
	"use strict";
...
//							// add custom filters
					...
					onActiveButtonPress: function(oEvent) {
						...
						if (oState.oMultipleViewsHandler.getMode() === "multi") {
							// in case of multiple tables, state of the button is handled in the multiple views helper class
							oState.oMultipleViewsHandler.setActiveButtonState(oEvent);
						} else {
					...
					// event handler for the close button found on the message strip
					onMessageCloseActionPress: function() {
						oState.oMultipleViewsHandler.onMessageCloseActionPress();
					}
				},
				formatters: {
