...
sap.ui.define([
	 ...
	 */
	var Viewer = Control.extend(NS.getName("Viewer"), /** @lends sap.ui.vk.Viewer.prototype */ {
		metadata: {
			library: NS.getName(),
			...
			aggregations: {
				 ...
				 */
				contentResources: {
					type: NS.getName("ContentResource"),
					forwarding: {
				...
				overlay: {
					type: NS.getName("Overlay"),
					multiple: false
				},

				progressIndicator: {
					type: NS.getName("ProgressIndicator"),
					multiple: false,
					visibility: "hidden"
				},
	...
	Viewer.prototype.getGraphicsCore = function() {
		return this._mainScene instanceof NS.getObject("dvl.Scene") ? this._mainScene.getGraphicsCore() : null;
	};
	...
	function isDvlViewport(vp) {
		...
		return vp instanceof NS.getObject("dvl.Viewport");
	}
	...
	Viewer.prototype._instantiateStepNavigation = function() {
		...
		if (this._mainScene instanceof NS.getObject("dvl.Scene")) {
			this._stepNavigation = new StepNavigation({
				showThumbnails: this.getShowStepNavigationThumbnails(),
				contentConnector: this._contentConnector
			});
		} else if (this._mainScene instanceof NS.getObject("threejs.Scene")) {
	...
	Viewer.prototype._showOverlay = function() {
		...
		if (oOverlayManager.changed) {
			...
			if (this.getEnableOverlay()) {
				sap.ui.require([ NS.getPath("Overlay") ], function(Overlay) {
	...
	Viewer.prototype.activateRedlineDesign = function(redlineElements) {
		var vp = this.getViewport().getImplementation();
		if (vp instanceof NS.getObject("threejs.Viewport") && vp.getCamera().getCameraRef().type === "PerspectiveCamera") {
			vp._viewportGestureHandler._activateRedline();
		}
		if (vp instanceof NS.getObject("dvl.Viewport") && vp.getViewInfo().camera && vp.getViewInfo().camera.projectionType === "perspective") {
			vp._activateRedline();
		}
		redlineElements = redlineElements || [];
		...
		} else if (vp instanceof NS.getObject("threejs.Viewport") && vp.getVisible()) {
	...
	Viewer.prototype.destroyRedlineDesign = function() {
		if (this.getRedlineDesign()) {
			var vp = this.getViewport().getImplementation();
			if (vp instanceof NS.getObject("dvl.Viewport")) {
				vp._deactivateRedline();
			} else if (vp instanceof NS.getObject("threejs.Viewport")) {
	...
	Viewer.prototype._handleContentReplaced = function(event) {
		...
		if (content && content instanceof NS.getObject("Scene")) {
			this._showViewport();
	...
	Viewer.prototype._handleContentChangesProgress = function(event) {
		...
		if (this._progressIndicator.getVisible()) {
			...
			this._progressIndicator.setDisplayValue(source + " " + phase + " " + (percentage ? Math.floor(percentage) + "%" : ""));
		}
	};
