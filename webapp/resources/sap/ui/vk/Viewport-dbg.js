...
sap.ui.define([
	 ...
	 * @since 1.50.0
	 */
	var Viewport = ViewportBase.extend(NS.getName("Viewport"), /** @lends sap.ui.vk.Viewport.prototype */ {
		metadata: {
			library: NS.getName(),
	...
	Viewport.prototype.getContents = function() {
		if (this._implementation) {
			return this._implementation.getContents();
		}
		return this.getAggregation("content");
	};

	Viewport.prototype.removeAllContents = function() {
		if (this._implementation) {
			this._implementation.removeAllContents();
		} else {
	...
	Viewport.prototype.activateView = function(view, notPlayingAnimations) {
		if (this._implementation && this._implementation.activateView) {
			this._implementation.activateView(view, notPlayingAnimations);
		}
	};
	...
	Viewport.prototype._setScene = function(scene) {
		if (scene instanceof Scene) {
			var sceneType = scene.getMetadata().getName(),
			    ...
			    reuseImplemenation = sceneType === NS.getName("dvl.Scene") && implementationType === NS.getName("dvl.Viewport") ||
									 sceneType === NS.getName("threejs.Scene") && implementationType === NS.getName("threejs.Viewport");
			...
			if (!reuseImplemenation) {
				...
				if (sceneType === NS.getName("dvl.Scene")) {
					newImplementationType = NS.getName("dvl.Viewport");
				} else if (sceneType === NS.getName("threejs.Scene")) {
					newImplementationType = NS.getName("threejs.Viewport");
				}
	...
	*/
	Viewport.prototype.resetCurrentView = function(includeAnimation) {
		if (this._implementation && this._implementation.resetCurrentView) {
			this._implementation.resetCurrentView(includeAnimation);
			return this;
		} else {
			jQuery.sap.log.error("no implementation");
			return this;
