...
sap.ui.define([
	...
	* @since 1.32.0
	*/
	var ViewStateManager = ViewStateManagerBase.extend(NS.getName("threejs.ViewStateManager"), /** @lends sap.ui.vk.threejs.ViewStateManager.prototype */ {
		metadata: {
		}
	});
	...
	ViewStateManager.prototype._setNodeHierarchy = function(nodeHierarchy) {
		...
		if (nodeHierarchy) {
			...
			allNodeRefs.forEach(function(nodeRef) {
				(nodeRef.visible ? that._initialState.visible : that._initialState.hidden).push(nodeRef);
			});
	...
	*/
	ViewStateManager.prototype.setVisibilityState = function(nodeRefs, visible, recursive) {
		...
		var changed = nodeRefs.filter(function(nodeRef) {
			return nodeRef ? nodeRef.visible != visible : false;
		}, this);
		...
		if (changed.length > 0) {
			changed.forEach(function(nodeRef) {
				...
				if (visible) {
					// Make sure all ancestor nodes are also visible but don't change state of other sibling nodes ('recursive' is set to false)
					var ancestors = this._nodeHierarchy.getAncestors(nodeRef);
					this.setVisibilityState(ancestors, true, false);
				}
			}, this);
