...
function(
	 ...
	 * @since 1.50.0
	 */
	var ContentManager = ContentManagerBase.extend(NS.getName("threejs.ContentManager"), /** @lends sap.ui.vk.threejs.ContentManager.prototype */ {
		metadata: {
			library: NS.getName()
		}
	});
		...
		var load = function() {
			...
			that._loadContentResources(scene, contentResources).then(
				...
				function(reason) { // onRejected
					var errMsg = typeof reason === "string" ? reason : reason.errorText;
					if (errMsg == null) {
						errMsg = getResourceBundle().getText(Messages.VIT37.summary);
					}
			...
			if (sourceType === "stream") {
				if (this.defaultCdsLoader == null) {
					...
					return new Promise(function(resolve) {
						var cdsModule = NS.getPath("threejs/ContentDeliveryService");
						sap.ui.require([
							cdsModule
						], function(Class) {
