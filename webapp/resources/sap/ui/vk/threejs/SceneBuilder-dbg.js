...
sap.ui.define([
	...
	var SceneBuilder = function(rootNode) {
		...
		this._sceneIdNodeMeshIdMap = new Map();	// map of scene id and map of mesh ids: sceneId and this._NodeMeshIdSubmeshIdsMap
	...
	SceneBuilder.prototype.setRootNode = function(rootNode, nodeId, sceneId, vkScene) {
		...
		if (sceneId) {
			...
			this._sceneIdNodeMeshIdMap.set(sceneId, this._NodeMeshIdSubmeshIdsMap);
	...
	SceneBuilder.prototype._getMaterialRef = function(materialId) {
		...
		var materialRef;
		if (material) {
			materialRef = material.getMaterialRef();
		}
		return materialRef;
	};
	...
	SceneBuilder.prototype._setType = function(node) {
		if (node.userData.treeNode) {
			if (node.userData.treeNode.contentType === "HOTSPOT") {
				node.userData.objectType = ObjectType.Hotspot;
			} else if (node.userData.treeNode.contentType === "PMI") {
				node.userData.objectType = ObjectType.PMI;
			}
		}
	 ...
	 * @param {any} sceneId The id of scene containing the node
	 * @returns {any} The result object contains these properties:<br/>
	...
	*/
	SceneBuilder.prototype.createNode = function(nodeInfo, parentId, sceneId) {
		...
		if (nodeInfo.transformType) {
			node.userData.transformType = nodeInfo.transformType;
	...
	SceneBuilder.prototype.applyNodeMaterialToSubmeshes = function(nodeId, sceneId) {
		...
		var nodeMaterial = this._getMaterialRef(node.userData.materialId);
		if (!nodeMaterial) {
	...
	SceneBuilder.prototype.updateMaterialInNode = function(nodeInfo, sceneId) {
		...
		if (node.userData.materialId) {
			nodeMaterial = this._getMaterialRef(node.userData.materialId);
		...
		if (node && node.children) {
			for (var i = 0; i < node.children.length; i++) {
				...
				if (nodeMaterial) {
					...
					var meshMaterial = this._getMaterialRef(child.userData.initialMaterialId);
	...
	SceneBuilder.prototype.getChildNodeIds = function(nodeId, sceneId, includeMeshNode) {
		...
		if (node && node.children) {
			for (var i = 0; i < node.children.length; i++) {
				var child = node.children[i];
				if (child.userData && child.userData && child.userData.treeNode && child.userData.treeNode.sid) {
	...
	SceneBuilder.prototype.setSubmesh = function(meshId, submeshInfo) {
		...
		if (submeshInfo.materialId) {
			material = this._getMaterialRef(submeshInfo.materialId);
			if (!material) {
		...
		if (geo) {
			if (geo.isPolyline) {
				...
				submesh = new THREE.LineSegments(geo, lineMaterial);
			} else if (material) {
			...
			if (geo.userData && geo.userData.noNormal && submeshInfo.materialId) {
	...
	SceneBuilder.prototype._insertSubmesh = function(nodeId, submeshId, sceneId) {
		...
		if (submesh.parent) {
			...
			if (submesh.userData) {
				if (submesh.userData.submeshInfo && submesh.userData.initialMaterialId) {
					var meshMaterial = this._getMaterialRef(submesh.userData.initialMaterialId);
					if (meshMaterial) {
		...
		/* if (node.renderOrder) {
			...
			var mMaterial = this._getMaterialRef(submesh.userData.initialMaterialId);
			if (mMaterial) {
				mMaterial.depthTest = false;
			}
		}*/

		this._setType(submesh);
	...
	SceneBuilder.prototype.createMaterial = function(materialInfo) {
		...
		var result = [];
		var material = this._getMaterialRef(materialInfo.id);
		...
		if (materialInfo.lineWidth > 0) {
			if (!material || !material.isLineBasicMaterial) {
				...
				vkMaterial.setMaterialRef(material);
				this._vkScene.setMaterial(materialInfo.id, vkMaterial);
			}
	...
	SceneBuilder.prototype.insertSequence = function(info) {
		...
		this._vkScene.createAnimationSequence(info.id, info.name, duration);
	...
	SceneBuilder.prototype.finalizePlaybacks = function() {
		...
		while (!next.done) {
			...
			for (var vi = 0; vi < viewGroup.views.length; vi++) {
				if (viewGroup.views[vi].id) {
					var view = this._views.get(viewGroup.views[vi].id);
					if (view) {
			...
			if (viewsInGroup.length > 0) {
				this._animationHelper.setInitialNodePositionsFromSubsequentViews(viewsInGroup, this._vkScene);
				this._animationHelper.setInitialNodePositionsFromPreviousViews(viewsInGroup, this._vkScene);
				// this._animationHelper.setInitialNodePositionsFromCurrenetViews(viewsInGroup, this._vkScene);
			}
			*/
		}
		/*
		var views = this._views.values();
		var nextView = views.next();
		while (!nextView.done) {
			var v = nextView.value;
			this._animationHelper.setInitialNodePositionsOnView(v, this._vkScene);
			nextView = views.next();
		}
		*/
		return this;
	...
	SceneBuilder.prototype.finalizeHighlightsInViews = function() {
		...
		var view, viewId;
		var viewEntries = this._views.entries();
		var nextView = viewEntries.next();
		while (!nextView.done) {
			view = nextView.value[1];
			viewId = nextView.value[0];
			nextView = viewEntries.next();
			if (view && view.highlightIdNodesMap) {
				...
				this._animationHelper.processHighlights(view, viewId, this._vkScene);
				this._animationHelper.setInitialNodePositionsOnView(view, this._vkScene, true);
			}
		}
		...
		while (!next.done) {
			...
			if (viewsInGroup.length > 0) {
				this._animationHelper.setInitialNodePositionsFromSubsequentViews(viewsInGroup, this._vkScene, true);
				this._animationHelper.setInitialNodePositionsFromPreviousViews(viewsInGroup, this._vkScene, true);
			}
		}
		return this;
	};
	...
	SceneBuilder.prototype.getSequence = function(sequenceId) {
		return this._vkScene.getAnimationSequence(sequenceId);
	};
	...
	SceneBuilder.prototype.cleanup = function() {
		...
		if (this._sceneIdTreeNodesMap) {
			this._sceneIdTreeNodesMap.clear();
		}
		if (this._sceneIdRootNodeMap) {
			this._sceneIdRootNodeMap.clear();
		}
		if (this._sceneIdNodeMeshIdMap) {
			this._sceneIdNodeMeshIdMap.clear();
		}
		...
		if (this._sceneIdViewGroupMap) {
			this._sceneIdViewGroupMap.clear();
		}
		if (this._sceneIdViewMap) {
			this._sceneIdViewMap.clear();
		}
		if (this._highlightStyles) {
