...
sap.ui.define([
	 ...
	 */
	var DetailView = BaseObject.extend(NS.getName("threejs.DetailView"), /** @lends sap.ui.vk.threejs.DetailView.prototype */ {
		metadata: {
			properties: {
				name: {
					type: "string",
				...
				type: {
					type: NS.getName("DetailViewType"),
					defaultValue: DetailViewType.DetailView
				},
				shape: {
					type: NS.getName("DetailViewShape"),
	...
	DetailView.prototype._render = function(renderer, camera, scene, boundingBox) {
				...
				root.userData._vkDynamicObjects.forEach(function(object) {
					if (object.visible) {
						if (object.isBillboard) {
							setObjectsVisibility(object.children, false); // hide screen-space objects
						} else {
		...
		scene.children.forEach(function(root) {
			if (root.userData._vkDynamicObjects) {
				root.userData._vkDynamicObjects.forEach(function(object) {
					if (object.visible && object.isBillboard) {
						setObjectsVisibility(object.children, true); // show screen-space objects
					}
				});
			}
		});
