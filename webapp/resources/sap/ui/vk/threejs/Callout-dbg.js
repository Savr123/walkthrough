...
sap.ui.define([
	 ...
	 */
	var Callout = Billboard.extend(NS.getName("threejs.Callout"), /** @lends sap.ui.vk.threejs.Callout.prototype */ {
		metadata: {
			properties: {
				anchorNode: {
	...
	Callout.prototype.setRenderOrder = function(value) {
		Billboard.prototype.setRenderOrder.call(this, value);
		this._lines.forEach(function(line) {
			line.renderOrder = value;
		});
		return this;
	};
	...
	Callout.prototype.setDepthTest = function(value) {
		...
		this._billboard.material.depthTest = value;
		this._lines.forEach(function(line) {
			line.material.depthTest = value;
		});
		return this;
	};
	...
	Callout.prototype._update = function(renderer, camera, rendererSize) {
		...
		if (rendererSize) {
			viewportSize.copy(rendererSize);
		} else {
	...
	Callout.prototype.addLeaderLine = function(vertices, targetNode, material, startPointStyle, endPointStyle, styleConstant, extensionLength) {
		...
		if (startPointStyle !== LeaderLineMarkStyle.None) {
			var mesh = this._createMarkMesh(material.color, lineStyle, startPointStyle, styleConstant);
			mesh.userData.targetVertex = polylineGeometry.vertices[ 0 ].clone();
			polylineMesh.userData.startPointMesh = mesh;
			node.add(mesh);
		}
