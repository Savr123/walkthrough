...
sap.ui.define([
	 ...
	 */
	var NodeHierarchy = NodeHierarchyBase.extend(NS.getName("threejs.NodeHierarchy"), /** @lends sap.ui.vk.threejs.NodeHierarchy.prototype */ {
		metadata: {
		},
	...
	NodeHierarchy.prototype.findNodesByName = function(query) {
		...
		if (query === undefined || query === null || jQuery.isEmptyObject(query)) {
			...
			if (searchModeFunction === undefined) {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT8.summary), Messages.VIT8.code, NS.getName("threejs.NodeHierarchy"));
			} else if (!Array.isArray(query.value) && typeof query.value !== "string") {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT6.summary), Messages.VIT6.code, NS.getName("threejs.NodeHierarchy"));
			} else {
