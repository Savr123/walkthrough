...
sap.ui.define([
	 ...
	 */
	var AnchorPointToolGizmo = Gizmo.extend(NS.getName("tools.AnchorPointToolGizmo"), /** @lends sap.ui.vk.tools.AnchorPointToolGizmo.prototype */ {
		metadata: {
			library: NS.getName()
		}
	});
	...
	AnchorPointToolGizmo.prototype.init = function() {
		...
		function createGizmoArc(axis, color, radius, segments) {
			var geometry = new THREE.TorusBufferGeometry(radius, window.devicePixelRatio / 288, 4, segments, Math.PI / 2);
			if (axis === 0) {
		...
		function createTouchArc(axis, radius, segments) {
			var geometry = new THREE.TorusBufferGeometry(radius, 24 / 144, 4, segments, Math.PI / 2);
			if (axis === 0) {
				geometry.rotateY(Math.PI / -2);
			} else if (axis === 1) {
				geometry.rotateX(Math.PI / 2);
		...
		// create 3 arcs
		for (var i = 0; i < 3; i++) {
			this._gizmo.add(createGizmoArc(i, 0xFF << 8 * (2 - i), 1, 32));
	...
	AnchorPointToolGizmo.prototype.show = function(viewport, tool) {
		...
		var enableRotation = this._tool.getEnableRotation();
		for (var i = 6; i < 9; i++) {// arcs
			this._gizmo.children[ i ].visible = this._touchAreas.children[ i ].visible = enableRotation;
		}

		var anchorPoint = viewport.getScene().getSceneRef().userData.anchorPoint;
		if (anchorPoint) {
			this._gizmo.position.copy(anchorPoint.position);
			this._gizmo.quaternion.copy(anchorPoint.quaternion);
		} else {
			viewport.getScene().getSceneRef().userData.anchorPoint = this._gizmo;
		}
	};
	...
	AnchorPointToolGizmo.prototype.highlightHandle = function(index, hoverMode) {
		...
		for (i = 6; i < 9; i++) {// arcs
			...
			arc.visible = this._tool.getEnableRotation() && (i === index || hoverMode);
