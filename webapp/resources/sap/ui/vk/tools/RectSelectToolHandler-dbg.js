...
sap.ui.define([
	...
	var RectSelectToolHandler = EventProvider.extend(NS.getName("tools.RectSelectToolHandler"), {
		metadata: {
			publicMethods: [
				"beginGesture",
				"endGesture",
		...
		if (this._gesture) {
			...
			if (this._selectionRect) {
				if (this._viewport) {
					var x = event.x - this._rect.x,
						y = event.y - this._rect.y;
					if (this._tool.isViewportType(NS.getName("dvl.Viewport"))) {
						this._tool.select(this._selectionRect.x1, this._selectionRect.y1, x, y, null, null);
					} else if (this._tool.isViewportType(NS.getName("threejs.Viewport"))) {
