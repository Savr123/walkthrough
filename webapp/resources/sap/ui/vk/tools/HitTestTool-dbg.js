...
sap.ui.define([
	 ...
	 */
	var HitTestTool = Tool.extend(NS.getName("tools.HitTestTool"), /** @lends sap.ui.vk.tools.HitTestTool.prototype */ {
		metadata: {
			properties: {
				/**
				 ...
				 */
				IdMode: {
					type: NS.getName("tools.HitTestIdMode"),
			...
			events: {
				...
				hit: {
					parameters: {
						...
						object: "any",
						point: "any",
						clickType: NS.getName("tools.HitTestClickType")
					}
				}
			}
		},
	...
	*/
	HitTestTool.prototype._prepare = function() {
		...
		if (okToExec) {
			if (this.isViewportType(NS.getName("dvl.Viewport")) && this._viewport._dvl) {
				this._dvlRendererId = this._viewport._dvlRendererId;
				this._dvl = this._viewport._dvl;
				okToExec = true;
			} else if (this.isViewportType(NS.getName("threejs.Viewport")) && (this._viewport._scene && this._viewport._scene.getSceneRef())) {
				okToExec = true;
			}
		}
		return okToExec;
	...
	HitTestTool.prototype.hitTest = function(x, y, scene, camera, clicktype) {
		if (this._prepare()) {
			...
			if (this.isViewportType(NS.getName("dvl.Viewport")) && this._viewport._dvl) {
				...
				return null;
			} else if (this.isViewportType(NS.getName("threejs.Viewport"))) {
