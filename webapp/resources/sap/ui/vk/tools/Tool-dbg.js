...
sap.ui.define([
	 ...
	 */
	var Tool = Element.extend(NS.getName("tools.Tool"), /** @lends sap.ui.vk.tools.Tool.prototype */ {
		metadata: {
			properties: {
				/**
				 *
	...
	Tool.prototype.setActive = function(value, activeTarget, gizmoContainer) {
		...
		if (value) {
			var _implementation = this.getViewportImplementation(activeTarget);
			...
			if (this.getGizmo()) {
				if (gizmoContainer) {
					...
					this.setAssociation("gizmoContainer", _implementation);
					_implementation.rerender();
				}
			}
			...
			this.setProperty("active", value, true);
			var vp = activeTarget || this._viewport;
			if (vp) {
				vp.setShouldRenderFrame(true);
			}
			this.fireEnabled({
				enabled: value,
