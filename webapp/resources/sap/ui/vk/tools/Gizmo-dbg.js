...
sap.ui.define([
	 ...
	 */
	var Gizmo = Control.extend(NS.getName("tools.Gizmo"), /** @lends sap.ui.vk.tools.Gizmo.prototype */ {
		metadata: {
			library: NS.getName()
		}
	});
	...
	Gizmo.prototype._updateEditingForm = function(active, axisIndex) {
		...
		if (domRef) {
			if (active && this._tool && this._tool.getShowEditingUI()) {
				...
				this._input.setValue(this._lastEditValue.toLocaleString("fullwide", { useGrouping: false  }));
				...
				var x = (viewportRect.right - viewportRect.left) * (position.x * 0.5 + 0.5);
				var y = (viewportRect.bottom - viewportRect.top) * (position.y * -0.5 + 0.5);

				domRef.style.left = Math.round(x + Math.sign(gizmoPosition.x) * -20) + "px";
