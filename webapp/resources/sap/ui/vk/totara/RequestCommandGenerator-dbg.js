...
sap.ui.define([
	...
	var RequestCommandGenerator = function() {
		...
		this.canGenerateCommand = function() {
			return meshIdSet.size > 0
				|| annotationIdArray.length > 0
				|| materialIdSet.size > 0
				|| imageIdSet.size > 0
				|| geometryIdArray.length > 0
				|| trackIdSet.size > 0
				|| sequenceIdSet.size > 0
				|| viewIdSet.size > 0
				|| highlightStyleIdSet.size > 0;
		};
		...
		this.generateRequestCommand = function(doNotEncode, token) {
			...
			if (meshIdSet.size > 0) {
				...
				command = Generator.createGetContentCommand(Command.getMesh, ids, token);
			} else if (annotationIdArray.length > 0) {
				ids = annotationIdArray.splice(0, annotationBatchSize);
				command = Generator.createGetContentCommand(Command.getAnnotation, ids, token, sceneId);
			} else if (materialIdSet.size > 0) {
				ids = sliceOneBatchFromSet(materialIdSet, materialBatchSize);
				command = Generator.createGetContentCommand(Command.getMaterial, ids, token);
			} else if (geometryIdArray.length > 0) {
				ids = geometryIdArray.splice(geometryIdArray.length - geometryBatchSize);
				command = Generator.createGetContentCommand(Command.getGeometry, ids, token);
				...
				command = Generator.createGetContentCommand(Command.getImage, ids, token);
				command.resources = ids;
			} else if (sequenceIdSet.size > 0) {
				ids = sliceOneBatchFromSet(sequenceIdSet, sequenceBatchSize);
				command = Generator.createRequestContentCommandWtihScenId(Command.getSequence, ids, sceneId, token);
			} else if (trackIdSet.size > 0) {
				ids = sliceOneBatchFromSet(trackIdSet, trackBatchSize);
				command = Generator.createGetContentCommand(Command.getTrack, ids, token);
				...
				if (id != undefined) {
					...
					command = { command : commandInStr, method : Command.getView };
					viewIdSet.delete(id);
				}
				...
				command = { command : commandInStr, method : Command.getHighlightStyle };
			}
