...
sap.ui.define([
	...
	CameraHandler.setCameraSingle = function(state, cameraInfo, sceneId) {
		...
		state.sceneBuilder.createCamera(cameraInfo, sceneId);
	...
	CameraHandler.setCamera = function(state, command) {
		...
		if (Array.isArray(cameraInfos)) {
			for (var i = 0; i < cameraInfos.length; i++) {
				CameraHandler.setCameraSingle(state, cameraInfos[i], command.sceneId);
			}
		}
