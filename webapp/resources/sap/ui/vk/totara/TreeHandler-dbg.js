		...
		if (command.sid) {
			...
			if (!root || root !== context.root) {
				...
				rootGroup.userData.treeNode = {
					sid: command.sid,
					name: context.root.name ? context.root.name : "root",
					skipIt: !context.root.name // If application didn't assign name to the root then ignore it in scene tree
				};

				if (context) {
					state.sceneBuilder.setRootNode(rootGroup, command.sid, command.sceneId, context.vkScene);
