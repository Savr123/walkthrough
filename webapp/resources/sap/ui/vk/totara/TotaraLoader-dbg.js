	...
	var TotaraLoader = function() {
		...
		function sendRequests() {
			...
			if (!somethingRequested) { // if nothing requested for individual context, load common ones. (mostly geometry)
				...
				// grab token from last the context. as token is for debugging/performance
				// and we normally load one model for debugging/performance, should be ok.
		...
		this.init = function(ssurl) {
				...
				if (!_pushMesh) {
					...
					if (meshIdsToRequest.size === 0) {
						...
						if (context.retrievalType === SceneStateContext.RetrievalType.Initial) {
							context.onInitialSceneFinishedCallbacks.execute(context.initialView);
						}
			...
			_commandProcessor.setCommandCallback(Command.notifyFinishedTree,
			...
			_commandProcessor.setCommandCallback(Command.setMesh,
				function(commandResult) {
					if (commandResult.error) {
						return;
					}
			...
			_commandProcessor.setCommandCallback(Command.setGeometry,
				function(commandResult) {
					if (commandResult.error) {
						return;
					}
			...
			_commandProcessor.setCommandCallback(Command.setAnnotation, function(commandResult) {
				var materialIds = _state.materialIdsToRequest;
				if (materialIds && materialIds.size) {
					materialIds.forEach(function(id) {
						_state.materialIdsRequested.add(id);
			...
			_commandProcessor.setCommandCallback(Command.setTree,
				function(command) {
			...
			_commandProcessor.setCommandCallback(Command.setViewGroup,
				function(result) {
					if (result.context) {
			...
			_commandProcessor.setCommandCallback(Command.notifyFinishedView,
				function(result) {
					if (result.error) {
						return;
					}
					...
					if (context) {
						...
						if (result.notifyFinishedTreeResult) {
							if (!result.notifyFinishedTreeResult.error){
								onFinishHierarchy(context);
							}
						}
			...
			_commandProcessor.setCommandCallback(Command.setMaterial,
				function(result) {
					if (result.error) {
			...
			_commandProcessor.setCommandCallback(Command.setImage,
				function(result) {
					if (result.error) {
						return;
					}
			...
			_commandProcessor.setCommandCallback(Command.setHighlightStyle,
				function(result) {
					if (result.error) {
						return;
					}
			...
			_commandProcessor.setCommandCallback(Command.setView,
				function(result) {
					if (result.error) {
						return;
					}
			...
			_commandProcessor.setCommandCallback(Command.setPlayback,
				function(result) {
					if (result.error) {
						return;
					}
					...
					if (result.sequenceIdSet.size > 0) {
						...
						if (context && result.onlyLoadingPlaybacks) {
							context.onSetPlaybackCallbacks.execute(result.view);
						}
					}
				});

			_commandProcessor.setCommandCallback(Command.setSequence,
				function(result) {
					if (result.error) {
						return;
					}
			...
			_commandProcessor.setCommandCallback(Command.setTrack,
				function(result) {
					if (result.error) {
						return;
					}
			...
			_commandProcessor.setCommandCallback(Command.notifyError,
				function(result) {
			...
			return new Promise(function(resolve) {
				if (!that._worker) {
					...
					var uri = new URI(sap.ui.require.toUrl(NS.getPath("totara/TotaraLoaderWorker.js")));
					if (uri.is("relative")) {
		...
		this.dispose = function() {
			...
			if (this._worker) {
				this._worker.postMessage({ method: "close" });
		...
		this.request = function(sceneVeId, contextParams, authorizationHandler) {
			...
			var selectField = contextParams.$select !== undefined ? contextParams.$select : "name,transform,meshId,annotationId,materialId,contentType,visible,opacity,renderOrder,entityId,highlightStyleId";
			...
			if (contextParams.activateView) {
				...
				viewOptions.context = sceneVeId;

				commandInStr = Generator.createGetViewCommand(viewOptions);
				context.initialViewId = contextParams.activateView;
				context.currentViewId = contextParams.activateView;
			} else {
				var options = {
					pushMaterials: true,
					pushMeshes: _pushMesh,
				...
				*/
				...
				commandInStr = Generator.createGetTreeCommand(options);
			}
		...
		this.requestViewGroup = function(sceneVeId, viewGroupId) {
			...
			var promise = new Promise(function(resolve, reject) {
				...
				var commandMethod = Command.getViewGroups;
		...
		this.requestView = function(sceneVeId, viewType, viewId, playbackIds) {
			...
			var promise = new Promise(function(resolve, reject) {
				...
				*/
				...
				if (viewType === "static") {
					...
					commandMethod = Command.getDynamicView;
				}
				...
				var setPlaybackCallback = function(resultView) {
					...
					logPerformance(context, "onSetPlayback", token);
				...
				_state.onSetSequenceCallbacks.detachAll();
				...
				_state.onSetTrackCallbacks.detachAll();
