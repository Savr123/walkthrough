sap.ui.define([
	"./ViewBuilder", "./CameraHandler", "./TotaraUtils", "./TreeHandler"
], function(ViewBuilder, CameraHandler, TotaraUtils, TreeHandler) {
	"use strict";
	...
	ViewHandler.setViewGroup = function(state, command) {
		...
		for (var vi = 0; vi < command.views.length; vi++){
			...
			if (view.thumbnailId !== undefined) {
				...
				state.viewIdAndThumbnailImageIdSceneIdMap.set(view.id, pair1);
	...
	ViewHandler.setView = function(state, view) {
		...
		if (view.sceneId) {
			...
			var cameraId;
			if (context.initialViewId === view.viewId) {
				if (view.camera) {
					cameraId = view.camera.id; // will be changed in setTree function, so to remember
				}
				result.setTreeResult = TreeHandler.setTree(state, view);
				state.viewIdSceneIdMap.set(view.viewId, view.sceneId);
			}

			view.id = view.viewId;
			var pair = state.viewIdAndThumbnailImageIdSceneIdMap.get(view.id);
			if (pair && pair.sceneId === view.sceneId) {
				view.thumbnailId = pair.thumbnailImageId;
				state.viewIdAndThumbnailImageIdSceneIdMap.delete(view.id);
			}
	...
	ViewHandler.setViewNode = function(state, command) {
		...
		if (command.sceneId) {
			...
			if (context.initialViewId === command.viewId) {
				result.setTreeNodeResult = TreeHandler.setTreeNode(state, command);
			}
	...
	ViewHandler.notifyFinishedView = function(state, command) {
		...
		if (command.sceneId) {
			...
			if (context.initialViewId === command.viewId) {
				result.notifyFinishedTreeResult = TreeHandler.notifyFinishedTree(state, command);
				context.initialViewId = null;  // make sure TreeHandler functions are only called for initial loading
			}
