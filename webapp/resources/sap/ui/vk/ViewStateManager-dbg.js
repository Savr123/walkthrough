...
sap.ui.define([
	 ...
	 * @since 1.32.0
	 */
	var ViewStateManager = ViewStateManagerBase.extend(NS.getName("ViewStateManager"), /** @lends sap.ui.vk.ViewStateManager.prototype */ {
		metadata: {
		}
	});
	...
	ViewStateManager.prototype._setScene = function(scene) {
		if (scene && scene instanceof Scene) {
			var sceneType = scene.getMetadata().getName(),
				...
				reuseImplemenation = sceneType === NS.getName("dvl.Scene") && implementationType === NS.getName("dvl.ViewStateManager") ||
					sceneType === NS.getName("threejs.Scene") && implementationType === NS.getName("threejs.ViewStateManager");

			if (!reuseImplemenation) {
				this._destroyImplementation();
				var newImplementationType;
				if (sceneType === NS.getName("dvl.Scene")) {
					newImplementationType = NS.getName("dvl.ViewStateManager");
				} else if (sceneType === NS.getName("threejs.Scene")) {
					newImplementationType = NS.getName("threejs.ViewStateManager");
				}
