...
sap.ui.define([
														...
														oData.getUserInput().then(function(oUserInput) {
															if (oUserInput) {
																...
																if (oUserInput.includeFilterSettings) {
																	ExportUtils.parseFilterConfiguration(this._getRowBinding(), this._getColumnLabel.bind(this)).then(function(oFilterConfig) {
	...
	SmartTable.prototype._triggerUI5ClientExport = function(mCustomConfig, mFilterConfig) {
		var oExportLibLoadPromise = this._loadExportLibrary();
		oExportLibLoadPromise.then(function() {
			var aColumns = this._oTable.getColumns(), i, iLen = aColumns.length, oColumn, oColumnData, sLabel, sPath, sTemplate, sAdditionalPath, oAdditionalColumn, sSplitLabel, oAdditionalProperty, sHAlign, nWidth, sType, inputFormat, oType, aSheetColumns = [];
	...
	SmartTable.prototype._createResponsivePopover = function() {
		this._oFilterButton = new ColumnPopoverActionItem({
			text: sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp").getText("SMARTTABLE_RP_FILTER"),
	...
	SmartTable.prototype._onCustomSort = function() {
		...
		this._oTable.fireEvent("sort", {
			...
			sortOrder: bAscending ? SortOrder.Descending : SortOrder.Ascending,
			columnAdded: false
		});
	...
	SmartTable.prototype._createAnalyticalColumn = function(oField, sId) {
		...
		oColumn = new AnalyticalColumn(sId, {
			...
			template: oField.template,
			groupHeaderFormatter: FormatUtil.getInlineGroupFormatterFunction(oField)
		});
		return oColumn;
	};
	...
	SmartTable.prototype._getTablePersonalisationData = function() {
		...
		var aSorters = [], aFilters = [], aExcludeFilters = [], oExcludeFilters, oGroupItem, oGroupSorter, aSortData, oColumn, oColumnData, sGroupPath, sAdditionalPath, sPath, mAdditionalSorters = {}, mExistingSorters = {}, sColumnsText = "", bIsTimeField;
		...
		if (this._isMobileTable && this._oCurrentVariant.group && this._oCurrentVariant.group.groupItems) {
			...
			if (sPath) {
				...
				if (oFieldMetadata) {
					sAdditionalPath = oFieldMetadata.unit || oFieldMetadata.description;
				...
				oGroupSorter = new Sorter(sGroupPath, oGroupItem.operation === "GroupDescending", function(oContext) {
					...
					if (fnGroupFunction) {
						if (sAdditionalPath) {
							sAdditionalValue = oContext.getProperty(sAdditionalPath);
						}
		...
		if (aSortData) {
			...
			if (aAdditionalSorterPaths.length) {
				aAdditionalSorterPaths.reverse().forEach(function(sAdditionalSorterPath) {
					aSorters.splice(mAdditionalSorters[sAdditionalSorterPath].index, 0, new Sorter(sAdditionalSorterPath, mAdditionalSorters[sAdditionalSorterPath].descending));
				});
			}
		}
