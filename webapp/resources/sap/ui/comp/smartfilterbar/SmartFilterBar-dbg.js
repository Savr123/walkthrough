...
sap.ui.define([
...
], function(jQuery, MessageBox, FilterBar, FilterGroupItem, FilterItem, library, AdditionalConfigurationHelper, ControlConfiguration, FilterProvider, GroupConfiguration, FormatUtil, PersonalizableInfo, SmartVariantManagement, ODataModelUtil, coreLibrary, VariantItem, AnnotationHelper, Context, VariantConverterFrom, Log, merge) {
	"use strict";
	...
	SmartFilterBar.prototype._getFlexControllerFactory = function() {
		...
		return sap.ui.getCore().loadLibrary('sap.ui.fl', {
			...
			return new Promise(function(fResolve) {
				sap.ui.require([
					...
					fResolve(FlexControllerFactory);
				});
			});
		});
	};
	...
	SmartFilterBar.prototype._waitForChanges = function() {
	   ...
	   return new Promise(function(fResolve) {
			this._getFlexControllerFactory().then(function(FlexControllerFactory) {
				  var oFlexController = FlexControllerFactory.createForControl(this);
				  if (oFlexController && oFlexController._oChangePersistence) {   // there is currently no method to check this on fl-side
					  oFlexController.waitForChangesToBeApplied(this).then(function() {
						  fResolve();
					  });
				  } else {
					  fResolve();
