...
sap.ui.define([
	...
	ODataControlFactory.prototype.getCustomDataConfiguration = function() {
	...
	ODataControlFactory.prototype._createEdmUOM = function() {
		var oEdmMeasureProperty = this.getEdmProperty(),
			bIsCurrency = this._oMetaData.annotations.uom && this._oHelper.oAnnotation.isCurrency(oEdmMeasureProperty, this._oMetaData.annotations.uom.property.property),
			mUOMAttributes = this._createEdmUOMAttributes({ // create the attributes for the amount text input field
		...
		this.addObjectBinding(mUOMAttributes, oObject);

		if (bRTLInTable) {
			mUOMAttributes.textDirection = "LTR";
		}

		var oAmountInputField = new Input(sSmartFieldID + "-input", mUOMAttributes);
		this.oMeasureField = oAmountInputField;
		...
		oCurrencySmartField.data("configdata", {
			"configdata": {
				...
				onText: function(oInnerControl) {
					oAmountInputField.setLayoutData(new FlexItemData({
						growFactor: 1
					}));
				...
				onInput: function(oInnerControl) {
					oAmountInputField.setLayoutData(that._getDynamicAmountInputFlexItemData({ control: oCurrencySmartField }));
		...
		oAmountInputField.addAriaLabelledBy(oCurrencySmartField);

		if (oCurrencySmartField.getVisible()) {
			oAmountInputField.addStyleClass("smartFieldPaddingRight");
		}

		oAmountInputField.addStyleClass("sapUiCompSmartFieldValue");

		var oHBox = new HBox({
			justifyContent: FlexJustifyContent.End,
			items: [oAmountInputField, oCurrencySmartField],
			fitContainer: true,
	...
	ODataControlFactory.prototype._createEdmUOMAttributes = function(mSettings) {
		mSettings = mSettings || {};
		var mAttributes = this.createUOMDefaultAttributes();
		var sValuePropertyMap = ODataControlFactory.getBoundPropertiesMapInfoForControl(Input.getMetadata().getName(), {
			propertyName: "value"
		})[0];
	...
	ODataControlFactory.prototype.createUOMDefaultAttributes = function() {
		return {
			textAlign: this._getEdmUOMTextAlignment(),
