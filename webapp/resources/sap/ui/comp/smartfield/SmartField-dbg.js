...
sap.ui.define([
	...
	var SmartField = Control.extend("sap.ui.comp.smartfield.SmartField", /** @lends sap.ui.comp.smartfield.SmartField.prototype */ {
		metadata: {
			...
			properties: {
				 ...
				 *
				 * // Extract of a definition of a <code>SmartField</code> control in an XML view:
	...
	SmartField.prototype.getUomProperty = function(sPropertyName) {
		var oFactory = this._oFactory,
			oFieldControl = oFactory && oFactory._oFieldControl,
		...
		if (this.isBound(sPropertyName) && oFieldControl) {
			var oUoMNestedSmartField = this._getEmbeddedSmartField(),
				oUoMFieldFactory = oUoMNestedSmartField && oUoMNestedSmartField.getControlFactory(),
				oMetaData = oUoMFieldFactory && oUoMFieldFactory.getMetaData();

			if (oMetaData) {
				var sUoMPropertyName = "uom" + sPropertyNameCapitalized,
					oBindingInfo = oFieldControl._oStoredBindings[sPropertyName],
					oUoMFormatter = oFieldControl["_get" + sPropertyNameCapitalized](oMetaData, oBindingInfo, sUoMPropertyName);

				oUoMFormatter.path();
				var vState = oUoMFormatter.formatter.call(this, bUoMPropertyValue);
				return vState;
			}
		}
	...
	SmartField.prototype.checkClientError = function() {
		...
		this.getInnerControls().forEach(function(oInnerControl) {
			this._checkErrors(oInnerControl, { checkClientErrorsOnly: true });
		}.bind(this));
	 ...
	 * <li>A validation error message is set on the text input field.</li>
	 ...
	 * @since 1.64
	 */
	SmartField.prototype.checkValuesValidity = function() {
		...
		return new Promise(function(fnResolve, fnReject) {
			var aPromises = this.getInnerControls().map(function(oInnerControl) {
				return this._checkErrors(oInnerControl);
			}, this);
	 ...
	 * @param {boolean} [mSettings.checkClientErrorsOnly=false] Indicates whether only client side errors are checked
	 * @returns {Promise} A fulfilled <code>Promise</code> object or a <code>Promise</code> object to be
	 * fulfilled if the value of every text input field is valid, otherwise a rejected <code>Promise</code>
	 * object or a <code>Promise</code> object to be rejected
	 * @private
	 */
	SmartField.prototype._checkErrors = function(oControl, mSettings) {
		mSettings = Object.assign({ checkClientErrorsOnly: false }, mSettings);
		...
		try {
			...
			if (sInternalType && oType) {
				...
				if (mSettings.checkClientErrorsOnly && oType.async) {
				...
				var vParseValueReturn = oType.parseValue(sValue, sInternalType, oBinding.getValue()),
