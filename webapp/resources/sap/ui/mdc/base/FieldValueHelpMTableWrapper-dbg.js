		...
		this._oPromises[sFieldPath][vValue] = new Promise(function(fResolve, fReject) {
			this._oTablePromise.then(function(oTable) {
				...
				if (oOutParameters) {
					...
					for ( var sOutParameter in oOutParameters) {
						if (!oInParameters || !oInParameters.hasOwnProperty(sOutParameter) || oInParameters[sInParameter] !== oOutParameters[sOutParameter]) {
				...
				try {
					...
					if (oFilterListBinding.isA("sap.ui.model.json.JSONListBinding")) { // TODO: find way unique for all ListBindings
						oFilterListBinding.filter(oFilter);
