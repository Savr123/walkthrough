			...
			if (wasSelected !== isSelected) {
				...
				if (wasSelected) {
					if (voGroup.minSel === "0" || voGroup.selected.length > 1) {
						// Deselect instance
						selectedIndex = voGroup.selected.indexOf(vo);
						deselected = voGroup.selected.splice(selectedIndex, 1);
					}
				} else if (voGroup.maxSel !== "0") {
					if (voGroup.maxSel === "1") {
						// Deselect all
						deselected = voGroup.selected.splice(0);
					}
					// Select instance
				...
				deselected.forEach(function(instance) {
					...
					// This needs to be done only to deselections arising as a side-effect of selection.
					// Selections already being taken care of.
