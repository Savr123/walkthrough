...
sap.ui.define([
	...
	function (
		...
		GanttPrinting.prototype.init = function () {
			...
			this._iFirstDayOfWeek = oCalendar._getLocaleData().getFirstDayOfWeek();
		...
		GanttPrinting.prototype.setGanttChart = function (oGanttChart) {
			...
			this._ganttChartClone.setSelectionPanelSize(oGanttChart._iLastTableAreaSize.toString() + "px");

			var oAxisTimeStrategy = oGanttChart.getAxisTimeStrategy();
			if (!oAxisTimeStrategy.isA("sap.gantt.axistime.FullScreenStrategy")) {
				var oChartScrollExt = oGanttChart._getScrollExtension();
				oChartScrollExt._updateVisibleHorizonForce();
			}
			...
			// set full screen strategy, we always need to see full graph
			if (!oAxisTimeStrategy.isA("sap.gantt.axistime.FullScreenStrategy")) {
				var oTotalHorizon = oAxisTimeStrategy.getTotalHorizon();
				var oFullScreenStrategy = new FullScreenStrategy({
				...
				this._ganttChartClone.setAxisTimeStrategy(oFullScreenStrategy);
			}
			...
			var oOrigRowsBinding = oGanttChart.getTable().getBinding("rows"),
		...
		GanttPrinting.prototype._getFirstDayOfNextWeek = function (oDate) {
			var iDaysToFirstDayOfWeek = (this._iFirstDayOfWeek + 7 - oDate.getDay()) % 7;
		...
		GanttPrinting.prototype._setGanttCloneDivWidth = function () {
			...
			this._oClonedGanttDiv.style.width = (this._getOriginalGanttChart()._iLastTableAreaSize + iTotalHorizonInPx + 10).toString() + "px";
		};
		...
		GanttPrinting.prototype._setGanttCloneDivHeight = function () {
			...
			this._oClonedGanttDiv.style.height = (iGanttHeight + 100).toString() + "px";
		};
		...
		GanttPrinting.prototype._drawClonedGantt = function () {
			...
			// FOR TESTING
			// this._oClonedGanttDiv.style.top = "500px";
