...
sap.ui.define([
	...
	GanttChartWithTable.prototype.init = function () {
		...
		this._oSplitter.attachResize(this.onSplitterResize.bind(this));
	...
	GanttChartWithTable.prototype.onSplitterResize = function(oEvent){
		...
		if (sDisplayType === GanttChartWithTableDisplayType.Both) {
			...
			if (bRefreshLastTableAreaSize && iNewSize) {
				this._iLastTableAreaSize = iNewSize;
			}
		}

		if (bResized) {
			// We need to determine if the resize happend by user resizing splitter or if the entire container changed size.
			// We cannot use exact equal, as somethimes rounding errors cause few pixels error
			if (almostEqual(aOldSizes.reduce(add), aNewSizes.reduce(add)) && aOldSizes[0] !== aNewSizes[0]) {
				this.setProperty("selectionPanelSize", iNewSize + "px", true);
	...
	GanttChartWithTable.prototype._onRedrawRequest = function(oEvent){
		...
		// initialRender need force update (redraw shapes completely)
		bForceUpdate = sReasonCode === "initialRender" ? true : bForceUpdate;
	...
	/**
	 * Redraw the chart svg if the surounding conditions changes, e.g zoom strategy updated, row binding context changed
	 * or while scrolling out of the buffer etc.
	 *
	 * @param {boolean} bForce Force redraw everything on right chart part.
	 * @private
	 */
	GanttChartWithTable.prototype.redraw = function(bForce) {
		if (bForce) {
	...
	GanttChartWithTable.prototype._draw = function() {
		var oScrollExtension = this._getScrollExtension();
	...
	GanttChartWithTable.prototype.onBeforeRendering = function(oEvent) {
		...
		this.getAggregation("_splitter").detachResize(this._onResize, this);
	};
	...
	GanttChartWithTable.prototype.onAfterRendering = function(oEvent) {
		...
		this.getAggregation("_splitter").attachResize(this._onResize, this);

		// at this point, there is no shape rendered at all, so jump to visible horizon
		// will change the zoomRate then trigger redraw
		// so sequence is: AfterRender -> Visible Horizon Change -->
		// -> Redraw -> Update scroll width -> render all shapes -> scroll gantt
		this.jumpToVisibleHorizon("initialRender");
	};
	...
	GanttChartWithTable.prototype._onResize = function() {
		var oSplitter = this.getAggregation("_splitter"),
			iFullWidth = this.getDomRef().clientWidth,
		...
		if (oSplitter.getContentAreas()[0] && oSplitter.getContentAreas()[0].getLayoutData()) {
			...
			if (iFullWidth < MIN_AREA_WIDTH * 2) {
				...
				iLeftWidth = oLeftPart.getDomRef().clientWidth;
	...
	GanttChartWithTable.prototype._setupDisplayType = function() {
		...
		if (sDisplayType === GanttChartWithTableDisplayType.Table) {
			...
			sTableSize = this._iLastTableAreaSize ? this._iLastTableAreaSize + "px" : this.getMetadata().getProperty("selectionPanelSize").getDefaultValue();
	...
	GanttChartWithTable.prototype.jumpToVisibleHorizon = function(sReason) {
		...
		this._updateVisibleHorizon(oTimeHorizon, sReason, this.getVisibleWidth());
	};
