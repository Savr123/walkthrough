...
sap.ui.define([
    ...
    AppState.prototype.createEmptyAppState = function (oComponent, bTransient, iPersistencyMethod, oPersistencySettings) {
        var sKey = this._getGeneratedKey(),
            ...
            aSupportedPersistencyMethods = this.getSupportedPersistencyMethods();

        if (iPersistencyMethod !== undefined) {
            var bIsSupported = aSupportedPersistencyMethods.find(function (element) {
                return element === iPersistencyMethod;
            });

            if (!bIsSupported) {
                throw new Error("PersistencyMethod " + iPersistencyMethod + " is not supported");
            }
        }
        ...
        if (bUseTransientAppState === true) {
            ...
            if (aSupportedPersistencyMethods.PersonalState !== undefined) {
                iPersistencyMethod = aSupportedPersistencyMethods.PersonalState;
            }
        }
    ...
    AppState.prototype.makeStatePersistent = function (sKey, iPersistencyMethod, oPersistencySettings) {
        return new Promise(function (fnResolve, fnReject) {
            this.getAppState(sKey).done(function (oAppState) {
                ...
                if (oAppState._iPersistencyMethod !== iPersistencyMethod ||
                    JSON.stringify(oAppState._oPersistencySettings) !== JSON.stringify(oPersistencySettings)) {
                    ...
                    this._saveAppState(sKey, oAppState._sData, oAppState._sAppName,
                        oAppState._sACHComponent, false, iPersistencyMethod, oPersistencySettings)
                        .done(function () {
                            fnResolve();
                        })
                        .fail(function () { // Failed to save the AppState object
                            fnReject();
                        });
                } else {
            ...
            }).fail(function () { // Failed to get the AppState object
                fnReject();
            });
        });
    };
