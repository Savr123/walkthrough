...
sap.ui.define([
    ...
    var Tile = Control.extend("sap.ushell.ui.launchpad.Tile", /** @lends sap.ushell.ui.launchpad.Tile.prototype */ {
        metadata: {
            library: "sap.ushell",
            properties: {
                long: // whether tile spans more than one column
                ...
                debugInfo: // technical information about the tile which is logged when the tile is clicked
                    { type: "string", group: "Misc", defaultValue: null },
                rgba: // the RGBA value of the tile
                    { type: "string", group: "Misc", defaultValue: null },
                animationRendered:
                    { type: "boolean", group: "Misc", defaultValue: false },
                isLocked:
                ...
                navigationMode:
                    { type: "string", group: "Misc", defaultValue: null },
                isDraggedInTabBarToSourceGroup: // in case of drag&drop in TabBar, this flag indicated that the tile is the one that was dragged between groups
                    { type: "boolean", group: "Misc", defaultValue: false }
            },
            aggregations: {
                ...
                if (oNavigationModeDiv == null) {
                    ...
                    if (sNavigationMode) {
                        ...
                        if (sNavigationModeTranslatedText) {
                            ...
                            Log.warning("could not get the navigation mode text of this tile to be added on the aria-labelledBby attribute");
                        }
                        // add it to the DOM
    ...
    Tile.prototype._redrawRGBA = function (args) {
        ...
        if (sRGBAvalue) {
            jqTile = jQuery(document.getElementById(this.getId()));
