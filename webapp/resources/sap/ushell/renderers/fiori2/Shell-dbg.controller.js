    ...
    sap.ui.controller("sap.ushell.renderers.fiori2.Shell", {
        ...
        handleDataLoss: function (newHash, oldHash) {
            ...
            if (sap.ushell.Container.getDirtyFlag()) {
                ...
                var sRestoreStrategy = this._getRestoreHashStrategy(sDirection, bWasHistoryEntryReplaced);
         ...
         *
         * @param {string} sDirection The navigation direction obtained via UI5 History API
         * @param {boolean} bNavToReplacedHash Whether the last navigation had replaced the history without adding a new entry.
         * @returns {string} The strategy to use to restore the previous shell hash.
         *   It's one of: "historyBack", "addHash", "replaceHash".
         */
        _getRestoreHashStrategy: function (sDirection, bNavToReplacedHash) {
            if (sDirection === "Backwards") {
                /*
                 * User had navigated via browser back button making the old hash disappear from the history.
                 * When the data loss dialog is cancelled the old hash should be restored by adding it back to the history.
                 */
                return "addHash";
            }

            /*
             * Here we handle the following two cases:
             *
             * 1. User had navigated forwards to an existing application or to a new application.
             *    This is possible by programmatic navigation or by forward navigation via browser button.
             *    When forwards navigation is made programmatically, it can be potentially made via replaceHash
             *    (without touching the history). In this case we need to restore via replaceHash, but if the
             *    entry was added to the history (setHash), we need to remove it by going back.
             *
             * 2. UI5 does not know if forwards or backwards was made.
             *    We here assume a forward navigation, and we restore based on bNavToReplacedHash.
             */
        ...
        _handleEmbeddedNavMode: function (sFixedShellHash, oParsedShellHash, oMetadata, oResolvedHashFragment) {
            ...
            // load Ui5ComponentLoader service because listeners are registered to that task which will in turn load the plugins.
            // MeArea, Search and others are implemented as plugin.
            sap.ushell.Container.getServiceAsync("Ui5ComponentLoader").then(function () {
        ...
        /**
         * sizeChange handler, trigger by the sap.ui.Device.media.attachHandler to handle header end ites overflow scenario
