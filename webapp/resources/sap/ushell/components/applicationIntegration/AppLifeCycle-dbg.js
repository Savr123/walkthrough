    ...
    function AppLifeCycle () {
        ...
        this.handleExitApplication = function (sFromId, oFrom, handleExitApplication) {
            ...
            if (sFromId && oFrom) { // FIXME oFrom.getApplicationType is not defined for Homepage and AppFinder in case disableHomeAppCache is true
                if (oFrom.getUrl() && BlueBoxHandler.getStateFul(oFrom.getUrl())) {
        ...
        this.storeApp = function (appId, oContainer, oTarget, sFixedShellHash) {
            if (!this.isAppInCache(appId)) {
                ...
                if (BlueBoxHandler.isTypeSupported(this.calculateAppType(oTarget))) {
        ...
        this.openApp = function (inAppId, oTarget, oShellHash, sFixedShellHash) {
            ...
            if (BlueBoxHandler.isTypeSupported(sAppType)) {
                oContainer = BlueBoxHandler.getStateFul(oTarget.url);

                if (!oContainer) {
                    var oHandler = BlueBoxHandler.getHandler();

                    if (oHandler && oHandler.setup) {
                        oHandler.setup(oTarget, appId);
                    }
        ...
        this.init = function (inAppState, oInViewPortContainer, inSRootIntent, inDisableHomeAppCache, oShellUIServiceChange, aActions, oCacheConfigurations) {
            ...
            DelegationBootstrap.init(this.registerShellCommunicationHandler);
        ...
        this.handleControl = function (sIntent, sAppId, oShellHash, oTarget, fnWrapper, sFixedShellHash) {
            var oInnerControl = this.getControl(sIntent),
                ...
                sAppType = oTarget.applicationType,
                bIsInitial = false,
                bDefaultFullWidth;
            ...
            bReuseStatefulContainer = BlueBoxHandler.isTypeSupported(sAppType);
            ...
            if (bReuseStatefulContainer) {
                var sToId = "application" + sAppId,
                    that = this,
                    sCurrentPageId = oViewPortContainer.getCurrentCenterPage(),
                    oCurrentPage = sap.ui.getCore().byId(sCurrentPageId);

                if (oCurrentPage.getApplicationType && !bIsInitial) {
                    this.handleExitStateful(sCurrentPageId, oCurrentPage).then(function () {
                        that.handleOpenStateful(bIsInitial, bIsInCache, sToId, oInnerControl, oTarget, sFixedShellHash);
                    });
                } else {
                    that.handleOpenStateful(bIsInitial, bIsInCache, sToId, oInnerControl, oTarget, sFixedShellHash);
                }
        ...
        this.postMessageToIframeApp = function (sServiceName, sInterface, oMessageBody, bWaitForResponse) {
            var fnCallBackHandler,
                ...
                aContainers = [];
            if (oContainer && (BlueBoxHandler.isCapabilitySupported(oContainer, sServiceName, sInterface) || Application.isAppTypeSupported(oContainer, sServiceName, sInterface))) {
                aContainers.push(
            ...
            if (!Application.isActiveOnly(sServiceName, sInterface)) {
                Storage.forEach(function (oEntry) {
                    if (oEntry && oEntry.value && oEntry.value.container) {
                        if (BlueBoxHandler.isCapabilitySupported(oEntry.value.container, sServiceName, sInterface) || Application.isAppTypeSupported(oEntry.value.container, sServiceName, sInterface)) {
                            aContainers.push(
                                Application.postMessageToIframeApp(oEntry.value.container, sServiceName, sInterface, oMessageBody, bWaitForResponse)
                            );
                        }
                    }
                });
        ...
        this.postMessageToIframeAppAccordingToPolicy = function (sServiceName, sInterface, oMessageBody,
            ...
            if (oContainer && Application.isAppTypeSupportedByPolicy(oContainer, oPolicy)) {
                aContainers.push(
                    Application.postMessageToIframeApp(oContainer, sServiceName, sInterface, oMessageBody, bWaitForResponse)
                );
            }
            ...
            if (!oPolicy.isActiveOnly) {
                Storage.forEach(function (oEntry) {
                    if (oEntry && oEntry.value && oEntry.value.container) {
                        if (Application.isAppTypeSupported(oEntry.value.container, sServiceName, sInterface)) {
                            aContainers.push(
                                Application.postMessageToIframeApp(oEntry.value.container, sServiceName, sInterface, oMessageBody, bWaitForResponse)
                            );
                        }
