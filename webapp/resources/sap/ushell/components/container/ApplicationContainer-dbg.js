...
sap.ui.define([
    "sap/ui/Device",
    "sap/ushell/utils",
    "sap/ushell/library", // required for "sap.ushell.components.container.ApplicationType.URL"
    ...
    */
    ...
    function getIFrame () {
        ...
        if (!oIFrame || oIFrame.tagName !== "IFRAME") {
            if (jQuery.sap.getUriParameters().get("sap-post") === "true" && oIFrame && oIFrame.getAttribute && oIFrame.getAttribute("sap-iframe-app") == "true") {
    ...
    function render (oRenderManager, oContainer, sApplicationType, sUrl, sAdditionalInformation) {
        ...
        //App Runtime: until URL template will be ready, we must have code
        ...
        if (oURI.query(true).hasOwnProperty("sap-isolation-enabled")) {
            ...
            if (sHash) {
                ...
                var sParams = oParser.paramsToString(oHash.params);
                if (sParams && sParams.length > 0) {
                    sUrl = sUrl.split("#")[0] + "&sap-startup-params=" + encodeURIComponent(sParams) + "#" + window.hasher.getHash();
                } else {
        ...
        if (jQuery.sap.getUriParameters().get("sap-post") === "true" && utils.isApplicationTypeEmbeddedInIframe(sApplicationType)) {
            ...
            if (aKeysArray.length > 0) {
                var oCAService = sap.ushell.Container.getService("CrossApplicationNavigation");
                ApplicationContainer.prototype._generateRootElementForIFrame(oRenderManager, oContainer);
    ...
    function buildHTMLElements (oRenderManager, oContainer, aAdditionalDataArray, aInfoArray, sUrl) {
        ...
        aInfoArray.forEach(function (item, index) {
            if (item === "sap-intent-param-data") {
                var aParams = aAdditionalDataArray[index][0].split("&");
                //"sap-intent-param" can hold more than one key-value pair. we need to create form element for each one of them
                aParams.forEach(function (sParam) {
                    statement = "<input name=\"" + sParam.split("=")[0] + "\" value=\"" + encodeXML(sParam.split("=")[1]) + "\"/>";
        ...
        oRenderManager
            .write("<iframe")
            .writeAttributeEscaped("name", oContainer.getId() + "-iframe")
            .writeControlData(oContainer)
            .writeAccessibilityState(oContainer)
            .addClass("sapUShellApplicationContainer")
            .writeClasses(oContainer)
        ...
        oRenderManager
            .write("<script>document.forms[\"" + sFormId + "\"].submit();</script>");
    }

    function generateRootElementForIFrame (oRenderManager, oContainer) {
