...
sap.ui.define([
    ...
    ActionMode.prototype.toggleActionMode = function (oModel, sSource, dashboardGroups) {
        ...
        // AnchorNavigationBar is not shown when there is only 1 visible group.
        // If there are more than 1 visible group, header of the first group is hidden
        if (iNumberVisibleGroup > 1) {
            aGroups[iFirstVisibleGroup].showGroupHeader = false;
        }

        /*
        Always show the group header in edit mode if the GridContainer is used.
        This is important because we need to show the "Add Tile" button in the group header.
        Otherwise the user would not be able to add a new tile as the PlusTile doesn't exist in the GridContainer.
        */
        var bGridContainerEnabled = Config.last("/core/home/gridContainer");
        if (!bTileActionModeActive && bGridContainerEnabled) {
            aGroups[iFirstVisibleGroup].showGroupHeader = true;
        }
...
sap.ui.define([
    ...
    "sap/ui/thirdparty/jquery",
    "sap/base/Log"
    // "sap/ushell/renderers/fiori2/AccessKeysHandler" // TODO: fix this resource route (proxy problem?)
], function (
    Config,
    ...
    ComponentKeysHandler.prototype = {
        ...
        goToLastVisitedTile: function (jqTileContainerToLookUnder, bLookInGivenGroup) {
            var jqDefaultContent,
                ...
                jqAllContent = jQuery(".sapUshellTile, .sapMGTLineMode");
            if (bLookInGivenGroup) {
                ...
                var jqTileContainerContent = jqTileContainer.find(".sapUshellTile, .sapMGTLineMode");
        ...
        _goToSiblingElementInTileContainer: function (sDirection, jqFocused) {
            ...
            if (jqTileContainerElement.length) {
                if (sDirection === "up") {
                    ...
                    jqFirstTileInTileContainer = jqTileContainer.find(".sapUshellTile, .sapUshellLinkTile").eq(0);
        ...
        setTileFocus: function (jqTile) {
            ...
            var jqAllContent = jQuery(".sapUshellTile, .sapMGTLineMode, .sapUshellLinkTile, .sapMGT").filter("[tabindex=0]").get();
            for (var i = 0; i < jqAllContent.length; i++) {
                jqAllContent[i].setAttribute("tabindex", "-1");
            }

            if (jqTile.hasClass("sapFCard")) {
                jqTile = jqTile.closest(".sapUshellCard");
            }
        ...
        _getFocusOnTile: function (jqFocused) {
            ...
            if (jqTile && jqTile.hasClass("sapUshellCard")) {
                jqTile = jqTile.children();
            }
        ...
        _homeEndButtonsHandler: function (selector, oEvent, jqFocused) {
            if (jqFocused.hasClass("sapUshellAnchorItem")) {
                ...
                this._preventDefault(oEvent);
                var jqTileToSelect = jQuery(".sapUshellTile").filter(":visible")[selector]();
        ...
        _getNextCatalog: function (sDirection, jqCurCatalog) {
            ...
            if (sDirection === "down" || sDirection === "right") {
                ...
                Log.error("Direction is unkown", sDirection, "sap.ushell.components.homepage.ComponentKeysHandler");
                return null;
            }
...
sap.ui.define([
    ...
    sap.ui.controller("sap.ushell.components.homepage.DashboardContent", {
        ...
        _scrollToGroupByName: function (sChannelId, sEventId, oData) {
            ...
            oGroups.forEach(oGroups, function (oGroup) {
        ...
        _scrollToGroup: function (sChannelId, sEventId, oData) {
            ...
            oTopElement.scrollIntoView(); // do scroll
            ...
            setTimeout(function () {
                ...
                if (oData.restoreLastFocusedTile) {
                    ...
                    ComponentKeysHandler.goToLastVisitedTile(jqTileContainer, bLookForLastVisitedInSameGroup);
                    ...
                    if (jqGroupContent.length) {
                        ComponentKeysHandler.setTileFocus(jqGroupContent);
                    }
                }
        ...
        var DashboardGroupsBox = baseObject.extend("sap.ushell.components.homepage.DashboardGroupsBox", {
            ...
            createGroupsBox: function (oController, oModel) {
                ...
                this.oGroupsContainer = new DashboardGroupsContainer("dashboardGroups", {
                    accessibilityLabel: resources.i18n.getText("DashboardGroups_label"),
                ...
                this.oGroupsContainer.addEventDelegate({
                    onsapskipback: function (oEvent) {
                        ...
                        if (jQuery(".sapUshellAnchorItem").is(":visible")) {
                            ComponentKeysHandler.goToSelectedAnchorNavigationItem();
                        } else {
                            AccessKeysHandler.sendFocusBackToShell(oEvent);
                        }
                    },
                            ...
                            if (jQuery(document.activeElement).closest(".sapUshellTileContainerHeader").length) {
                                ...
                                if (isCurrentElementTitle && !jqChildActions.length || document.activeElement.id === jqChildActions.last()[0].id) {
                                    if (jqCurrentTileContainer.find(".sapUshellTile, .sapUshellLink, .sapFCard").is(":visible")) {
                                        oEvent.preventDefault();
                                        ComponentKeysHandler.goToLastVisitedTile(jqCurrentTileContainer, true);
                                        return;
                                    }
                                }
...
sap.ui.define([
	...
	"sap/ushell/components/homepage/FLPAnalytics.js":function(){// ${copyright}
