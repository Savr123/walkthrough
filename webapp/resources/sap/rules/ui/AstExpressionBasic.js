	...
	var AstExpressionBasic = ExpressionBase.extend("sap.rules.ui.AstExpressionBasic", {
		...
		_createSpanItem: function (oItem) {
			...
			if (oItem.tokenType === Constants.TERM) {
				...
				if (objectFromReference) {
					...
					} else if((oItem.resultDataObjectId !== "" && oItem.resultDataObjectId !== undefined && !this.ruleWithElementDO) || (oItem.tokenType === "ID" && oItem.dataObjectType != Constants.Element && !this.ruleWithElementDO && !this.isCountFunctionSelected)){
                        oItem.text = objectFromReference + Constants.DOT;
                    } else {
		...
		_setUpEventHandlers: function () {
			...
			this._input.on('keydown', function (event) {
				...
				} else if (event.key == "Control" || event.key == 'keyup') {
					that.ctrlPressed = true;
				} else if (that.ctrlPressed && event.key == " " || that.ctrlPressed && event.type == 'keydown') {
		...
		_isLastTokenSpaceItem: function () {
			...
			if (this._uiModel && this._uiModel.length > 0) {
				...
				} else if (oLastItem && oLastItem.tokenType !== "A" && oLastItem.tokenType !== "ID" && oLastItem.reference === undefined){
					isLastTokenSpace = false;
				}
			}
		...
		_updateUiModel: function () {
			...
			if (this.getDomRef("input")) {
				...
				for (var iterator = 0; iterator < children.length; iterator++) {
					var childSpan = $("#" + children[iterator].id);
					var childSpanData = childSpan.data("customData");
					childSpanData.text = children[iterator].textContent;
					var childSpanReference = children[iterator].getAttribute("reference");
					if (childSpanReference) {
						childSpanData.reference = childSpanData.tokenType !== Constants.GEOBUSINESSDATATYPE  ? childSpanReference : undefined;
