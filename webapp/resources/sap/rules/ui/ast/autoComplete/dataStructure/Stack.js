	...
	function (TermNode, Constants, BaseStack, FunctionProvider, OperatorProvider, FunctionalStack, ArrayAndRangeStack, OperatorNode,
		...
		Stack.prototype.push = function (oToken) {
			...
			if (type == Constants.AND || type == Constants.OR) {
				if (this.getHasOpenParenthesis() || this._doesStackContainsFunctions(this)) {
					this._getTopStackRecursively(this.getTop()).empty();
				} else {
					this.empty();
				}
				return {
			...
			case Constants.WINDOWANDADVANCED:
				...
				node = new WindowAdvancedStack(this.getTermPrefixId());
		...
		Stack.prototype._handleRightParenthesisToken = function (oToken) {
			if (this.getHasOpenParenthesis() == true) {
				...
				} else if (this._stackContainsComparisionStack(this._top)) {
					var oComparisionStack = this._getComparisionStack(this._top);
		...
		Stack.prototype._stackContainsComparisionStack = function (node) {
			...
			if (operatorNode && "getOperator" in operatorNode && operatorNode.getOperator() && "getSupportedFunctions" in operatorNode.getOperator() &&
				...
				for (var lIterator = 0; lIterator < aSupportedFunctions.length; lIterator++) {
					if (aSupportedFunctions[lIterator].name.toLocaleUpperCase() == topNode.getName()) {
						cText = prevNode.getName() + " " + operatorNode.getOperator().name + " " + topNode.getName();
						cNode.setName(cText).setLabel(cText);
						cNode.setDataObjectType(aSupportedFunctions[lIterator].dataObjectType);
