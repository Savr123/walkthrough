sap.ui.define([
	...
	"sap/rules/ui/ast/autoCompleteContent/AutoSuggestionWindowFunctionPanel",
	...
	AutoSuggestionFunctionalOperatorPanel, AutoSuggestionAdvancedFunctionPanel, AutoSuggestionWindowFunctionPanel,
	...
	var AutoCompleteSuggestionContent = Control.extend("sap.rules.ui.AutoCompleteSuggestionContent", {
		...
		_getAutoSuggestionList: function () {
			...
			if (this.autoSuggestions && this.autoSuggestions.categories && this.autoSuggestions.categories.terms) {
			...
			if (this.autoSuggestions && this.autoSuggestions.categories && this.autoSuggestions.categories.functions && !this._enableAggregateFunctionVocabulary) {
				this.fuctions = this.autoSuggestions.categories.functions;
				if (this.fuctions.window) {
					var WindowFunctionsPanel = new sap.rules.ui.ast.autoCompleteContent.AutoSuggestionWindowFunctionPanel({
						reference: that._reference,
						data: that.fuctions.window,
					});
					this.getAggregation("mainLayout").addItem(new sap.m.CustomListItem({
						content: WindowFunctionsPanel,
					}));
				}
			}
			...
			// Select functions
			if (this.autoSuggestions && this.autoSuggestions.categories && this.autoSuggestions.categories.functions && !this._enableAggregateFunctionWhereClause) {
				this.fuctions = this.autoSuggestions.categories.functions;
